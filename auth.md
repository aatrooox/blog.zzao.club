...
#### 错误代码定义（`/shared/utils/apiCodes.ts`）
...
### 1. API 响应处理器 (`/server/utils/handler.ts`)
...
### 3. 鉴权中间件 (`/server/middleware/2.auth0.ts`)
...
### 4. Token 管理 (`/server/utils/token.ts`)
...
### 2. 用户状态管理 (`/composables/useUser.ts`)
...
### 3. 鉴权组合式函数 (`/composables/useAuth.ts`)
...
通过遵循本文档的规范，可以确保系统的安全性和一致性，同时提供良好的开发体验。
